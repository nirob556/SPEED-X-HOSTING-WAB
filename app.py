...
app.secret_key = "@NIROB_BBX" 
UPLOAD_FOLDER = 'user_bots'
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

# --- ADMIN LOGIN CREDENTIALS ---
ADMIN_USER = "@NIROB_BBZ"
ADMIN_PASS = "NIROB"
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users 
                 (firebase_uid TEXT PRIMARY KEY, email TEXT, photo_url TEXT, 
                  plan_type TEXT DEFAULT 'Free', bot_limit INTEGER DEFAULT 1, 
    ...
    c.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('vip_price', '500')")
    c.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('premium_price', '250')")
    c.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('upi_id', '@NIROB_BBX')")
...
def login():
    if request.method == 'POST':
        ...
        if user and user['is_banned']:
            conn.close()
            return render_template('login.html', error="🚫 YOU ARE BANNED BY DPX OWNER")
        ...
        if not user:
            ...
            conn.execute('UPDATE users SET email=?, photo_url=? WHERE firebase_uid=?', (email, photo, uid))
...
def dashboard():
    ...
    conn = get_db()
    user = conn.execute('SELECT * FROM users WHERE firebase_uid = ?', (uid,)).fetchone()
    bots = conn.execute('SELECT * FROM bots WHERE firebase_uid = ?', (uid,)).fetchall()
...
def upload_bot():
    if 'user' not in session: return jsonify({'error': 'Login First'}), 401
    uid = session['user']
    
    conn = get_db()
    user = conn.execute('SELECT * FROM users WHERE firebase_uid = ?', (uid,)).fetchone()
    current_bots = conn.execute('SELECT COUNT(*) FROM bots WHERE firebase_uid = ?', (uid,)).fetchone()[0]
    ...
    if os.path.exists(global_req_path):
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "--no-warn-script-location", "-r", global_req_path], cwd=working_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            flash("Global dependencies (Base Modules) installed.", "info")
        except Exception as e:
            flash(f"⚠️ Global dependencies installation failed. Bot may not run.", "warning")
    ...
    if os.path.exists(local_req_path):
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "--no-warn-script-location", "-r", local_req_path], cwd=working_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            flash("Local dependencies installed.", "success")
        except Exception as e:
            flash(f"⚠️ Local dependencies installation failed. Check requirements file.", "warning")
...
def bot_action(action, bot_id):
    ...
    if action == 'start':
        if bot['status'] != 'running':
            ...
            conn.execute('UPDATE bots SET status=?, pid=? WHERE id=?', ('running', proc.pid, bot_id))
        ...
        if bot['status'] == 'running':
            if bot_id in running_processes:
                try: running_processes[bot_id].terminate()
                except: pass
        ...
        try: shutil.rmtree(bot['extract_path'], ignore_errors=True)
        except: pass
...
def file_manager(bot_id):
    ...
    bot = conn.execute('SELECT * FROM bots WHERE id = ?', (bot_id,)).fetchone()
    conn.close()
    if not bot: return jsonify({'files': []})
...
def update_settings():
    ...
    conn = get_db()
    conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('vip_price', ?)", (vip_price,))
    conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('premium_price', ?)", (premium_price,))
...
def admin_update_user():
    ...
    if action == 'ban':
        conn.execute('UPDATE users SET is_banned=1 WHERE firebase_uid=?', (uid,))
        bots = conn.execute('SELECT id FROM bots WHERE firebase_uid=?', (uid,)).fetchall()
        for b in bots: bot_action('stop', b['id'])
        flash(f"User {uid[:8]} BANNED.", "warning")
        
    elif action == 'unban':
        conn.execute('UPDATE users SET is_banned=0 WHERE firebase_uid=?', (uid,))
        flash(f"User {uid[:8]} UNBANNED.", "success")
        
    elif action == 'set_plan':
        plan = request.form.get('plan') 
        limit = request.form.get('limit') 
        conn.execute('UPDATE users SET plan_type=?, bot_limit=? WHERE firebase_uid=?', (plan, limit, uid))
        flash(f"User {uid[:8]} plan set to {plan} with {limit} bots.", "success")
